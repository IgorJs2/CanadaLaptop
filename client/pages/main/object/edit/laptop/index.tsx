...
import {fetchLaptopList, fetchLaptopsFromIdArray} from "../../../../../store/action-creators/laptop";
...
const Index = () => {
    ...
    const {fetchLaptopsFromIdArray, fetchLaptopEditFromList} = useAction()
    ...
    const [submitedData, setSubmitedData] = useState<ILaptopEditData[]>([])

    const submitHandler = (form: ILaptopEditData) => {
        if (form && form._id) {
    ...
    const deleteHandler = (e: React.MouseEvent<HTMLElement>) => {
        const id = e.currentTarget.getAttribute("data-id")
        if (id) {
            const array_with_removed_id = [...Laptops].map((item) => item._id).filter((i) => i !== id)
            setChecked([...checked.filter((i) => i !== id)])
            setSubmited([...submited.filter((i) => i !== id)])
            setSubmitedData([...submitedData.filter((i) => i._id !== id)])
            fetchLaptopsFromIdArray(array_with_removed_id, "")
        }
    }
    ...
    const submitEditHandler = (isAllSubmitted: boolean) => {
        ...
        fetchLaptopEditFromList(submitedData)
    }
    ...
    return (
        <MainLayouts active={1}>
            ...
            <EditListModal visible={visibleEditList} onClick={() => setVisibleEditList(false)} type={item_types.Laptop} checked={checked}
...

                    {
                        Laptops && Laptops[0] ? (
                            <div className="my-5 mx-5 flex justify-end">
                                ...
                                <Button color="success" variant="outlined" className="mx-2" onClick={() => submitEditHandler(submitedData.length === Laptops.length)}>Send edit items</Button>
                            </div>
                        ) : <></>
                    }
                </div>
...
export const getServerSideProps = wrapper.getServerSideProps((store: any) => async ({req, res}) => {
    try {
        ...
        if (id_array) {
            await store.dispatch(fetchLaptopsFromIdArray(id_array, token))
            removeCookies("id_array_to_edit", {req, res})
        }


        await store.dispatch(fetchCurrentUser(token))
        await store.dispatch(fetchLaptopList({} as ILaptopFilterData, token))

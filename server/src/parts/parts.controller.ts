...
export class PartsController {
    ...
    @UseGuards(TypesGuard)
    @Get("by_id")
    async getPartsById (@Query("id_array") id_array: string[]) {
        const response = await this.PartsService.getPartsById(id_array)
        return response
    }
    ...
    @UseGuards(TypesGuard)
    @Get("list")
    async getPartList (@Query("filter") filter: string) {
        const response = await this.PartsService.getPartList(filter)
        return response
    }

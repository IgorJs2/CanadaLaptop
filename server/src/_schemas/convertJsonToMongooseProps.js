            ...
            if(i % 2 === 0){
                if(e[`Текстовая область ${i}`]){
                   if(e[`Текстовая область ${i+1}`].match(new RegExp("objectid", "i")) && e[`Текстовая область ${i+1}`] !== "_id"){
                        value += ` \n@Prop() \n ${e[`Текстовая область ${i}`] + ": " + "{type: mongoose.Schema.Types.ObjectId, ref: \"\"}"} \n`
                    }
...
filtered.forEach((e, i) => {
    ...
    const ts = "import { Prop, Schema, SchemaFactory } from '@nestjs/mongoose';\n" +
        ...
        "\n" +
        `export type ${NamedByRules}ModelDocument = ${NamedByRules} & Document;\n` +
        "\n" +
        "@Schema()\n" +
        `export class ${NamedByRules} {\n` +
        ...
        "\n" +
        "\n" +
        `export const ${NamedByRules}ModelSchema = SchemaFactory.createForClass(${NamedByRules});`

        fs.writeFile(`${name}/${name}.schema.ts`, ts, function(err) {
            if (err) {
                console.log(err);
            }
        });
